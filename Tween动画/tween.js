...
(function() {
	var zfEffect = {
		...
		Bounce: {
			easeIn: function(t, b, c, d) {
				return c - zfEffect.Bounce.easeOut(d - t, 0, c, d) + b;
			},
			...
			easeInOut: function(t, b, c, d) {
				if (t < d / 2) {
					return zfEffect.Bounce.easeIn(t * 2, 0, c, d) * .5 + b;
				}
				return zfEffect.Bounce.easeOut(t * 2 - d, 0, c, d) * .5 + c * .5 + b;
			}
		},
		//二次方的缓动
		Quad: {
	...
	function move(curEle, target, duration, effect, callback) {
		...
		if (typeof effect === 'number') {
			...
			ary = str.split('-');
			tempEffect = ary.length >= 2 ? zfEffect[ary[0]][ary[1]] : zfEffect[ary[0]];
		} else if (effect instanceof Array) { //当effect传的是数组，进行拼接      传的格式['Bounce','easeInOut']
			var ary1 = effect[0];
			var ary2 = effect[1];
			tempEffect = effect.length >= 2 ? zfEffect[ary1][ary2] : zfEffect[ary1];
